...
class MechSystem(object):
    def __init__(self, alpha, beta, Omega, y_init, T, method=None, nosc=3):
        ...
        self.u_init = y_init
        self.T = T
        self._g = lambda y: y[:, :nosc].dot(alpha)
        self._G = lambda y: np.array(alpha)
    ...
    def en_err(self, y, t=0):
        ...
        V = lambda x: -sum(self.Omega2.dot(cos(x).T), axis=0)
...
class MechSystemSolver(object):
    """
    Class for solving problems of the class MechSystems
    """
    def __init__(self, problem, dt, solver_class, tol=1.0E-14, M=100, var=True, store=False):
        self.f, self.dt, self.solver_class = problem, dt, solver_class
        self.w_values = w_values
        self.var = var
        self.store = store
        self.tol, self.M = tol, M
        ...
        if self.solver_class in [ODESolver.ConformalStormerVerlet, ODESolver.ForwardEuler]:
            ...
            NameError('Unknown solver class - %s' % alg(solver_class))
        ...
        for k in range(self.n):
            ...
            for w_val in self.w_values:
                ...
                while abs(Delta_LambdaX) > self.tol and m <= self.M:
                    X = X -np.transpose(self.f._G(y_[0]))*Delta_LambdaX
                ...
                Delta_LambdaU, U, m = self.f._G(y_[1]).dot(y_[1, nosc:]), y_[1, nosc:], 0
                if self.store: self.info.append((U, Delta_LambdaU, m))

                while abs(Delta_LambdaU) > self.tol and m <= self.M:
                    U = U -np.transpose(self.f._G(y_[1]))*Delta_LambdaU

                    m += 1
                    Delta_LambdaU = self.f._G(y_[1]).dot(U)
                    if self.store: self.info.append((U, Delta_LambdaU, m))
    ...
    def var_solve(self):
        if not self.var:
            ...
            n = int(round(self.f.T/self.dt))
            nosc = self.f.nosc
            self.dpsi = np.zeros((n+1, 2*nosc, 2*nosc))
    ...
    def plot(self, tile=111):
        "plot the results"
        fig = figure()
        ax = fig.add_subplot(tile)
        if not self.f.beta:
            plot_data(ax, self.t_points, np.array([reshape(self.sym_error[-1:],(self.n+1,)), self.f.en_err(self.y), self.f._g(self.y), self.y[:, self.f.nosc:].dot(self.f._G(self.y))]).T, True, True)
            ...
            plot_data(ax, self.t_points, np.array([reshape(self.sym_error[-1:],(self.n+1,)), self.f._g(self.y), self.y[:, self.f.nosc:].dot(self.f._G(self.y))]).T, True, True)
...
#%% Parameters
nosc = 5
Omega = list(linspace(1,5,num=nosc))
...
T_final = 1
dt_space = concatenate(([], linspace(0.05, 0.2, num=5)))
...
def demo1():
    ...
    for solver_class in registered_solver_classes:
        eng_error = []
        for dt in dt_space:
            problem = MechSystem(alpha=alpha, beta=beta, Omega=Omega,\
                                   y_init=y_init, T=T_final,\
                                    method=solver_class, nosc=nosc)
            MSsolver = MechSystemSolver(problem=problem, dt=dt, solver_class=solver_class,\
                                      tol=1.0E-14, M=100, var=True, store=False)
            MSsolver.solve()
            
            eng_error.append(MSsolver.eng_error)
        
        # Estimate Convergence rate r and coefficient C
        if not beta and dt_space.size > 1:
    ...
    if not beta and dt_space.size > 1:
        ...
        # temp = np.asarray([dt_space, [np.float('nan')]+r_values[0], [np.float('nan')]+r_values[1]]).T
